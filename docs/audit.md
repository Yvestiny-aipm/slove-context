...
不是鉴权、队列或真实模型客户端。节点 2.1 的 Story Project / Story Spec 写操作、节点 2.2 的实体 / 证据 / Canon 事实创建、批准与废弃、节点 2.3 的 Canon Snapshot 创建与冻结、节点 3.1 的 Scene Card 创建 / 改草稿 / 批准 / 设依赖、节点 3.2 LLM Gateway 在重试循环结束后的一次审计（若注入了 `AuditWriter`），节点 3.3 Scene Plan 作业的创建 / 状态转换 / 计划落库，以及节点 3.4 Scene Draft 作业的创建 / 状态转换 / 草稿落库，复用本接口。Spec 批准不是 Canon 批准。快照冻结不是 Canon 批准。批准 Scene Card 不是 Canon 批准，不写 Canon。生成 Scene Plan / Scene Draft 不是 Canon 批准，不写 Canon。Gateway 不写 Canon。证据正文与完整 Prompt 不得写入 `before_json` / `after_json`。Scene Draft 审计只存哈希与引用，不存完整散文。
