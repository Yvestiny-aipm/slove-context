...
不是鉴权、队列或模型网关。节点 2.1 的 Story Project / Story Spec 写操作、节点 2.2 的实体 / 证据 / Canon 事实创建、批准与废弃、以及节点 2.3 的 Canon Snapshot 创建与冻结，复用本接口。Spec 批准不是 Canon 批准。快照冻结不是 Canon 批准。证据正文不得写入 `before_json` / `after_json`。
...
- 用户鉴权、队列、模型客户端 / 网关
- 自动批准、多项目
- Scene Card、Context Pack、生成器（节点 3.1 及以后）
