...
| `tests/` | `/healthz`、request_id、审计写入、Story Project / Spec、Canon、Snapshot、Scene Card、LLM Gateway、Scene Plan / Scene Draft 作业、Candidate Change 抽取与人类批准 / 提交、Scene / Chapter 摘要、Validation Run、Repair Task、Context Pack 组装、Outline Revision、Style Guide / Style Sample、Style Validation、审校队列、本地作业队列 / Worker、Agent 注册表 / 权限的进程内测试；可收集 `contracts/` 测试 | 不需要 live Postgres；无真实模型调用；Gateway / 作业测试只用 Fake / 夹具 |
...
## 3. 本仓库包含（节点 1.1 + 1.2 + 1.3 + 2.1 + 2.2 + 2.3 + 3.1 + 3.2 + 3.3 + 3.4 + 4.1 + 4.2 + 4.3 + 5.1 + 5.2 + 6.1 + 6.2 + 7.1 + 7.2 + 7.3 + 8.1 + 8.2）
...
- Agent 注册表 / 权限：七类 Agent（Outline / Draft / Extractor / Consistency / Style / Repair / Human Approver）。每条记录含 input / output schema、allowed tools、forbidden operations、model config、prompt version、timeout、cost cap。服务层 `PermissionGuard` 再检查；未授权 403。Agent Run 归档 input_ref / output_ref / tool_calls / cost / duration / error，可回放。任何 Agent（含 Worker / system）不得绕过 Approval 写 Canon。Human Approver 是唯一 Canon 批准角色。提交 Canon 仍走 4.2。无 DAG。无生产 seed-status。
- 进程内测试：`/healthz`、request_id、审计写入与脱敏、Story Project / Spec、Canon、Snapshot、Scene Card、LLM Gateway、Scene Plan / Scene Draft 作业、Candidate Change 抽取与人类批准 / 提交、Scene / Chapter 摘要、Validation Run、Repair Task、Context Pack 组装、Outline Revision、Style Guide / Style Sample、Style Validation、审校队列、本地作业队列 / Worker、Agent 注册表 / 权限（以及已有的 contracts 校验）。不连 Postgres，不调用真实模型。

## 4. 本节点明确不是

下列项不属于节点 8.2，不得当作本节点已交付：

- 节点 8.3 DAG、8.4 批量、节点 9.x。
