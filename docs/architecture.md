# 架构（节点 1.1 骨架 + 节点 1.2 本地环境 + 节点 1.3 审计日志）

本文冻结本地单体仓库的目录边界。节点 1.2 补可运行的 Postgres 与 FastAPI 健康检查。节点 1.3 只加请求 `request_id`、JSON 结构化日志、以及 `audit_events` 迁移与通用写入接口。  
...
节点 1.1 提供目录与约定。节点 1.2 增加可启动的 Postgres 与仅含 `/healthz`、`/version` 的 FastAPI。节点 1.3 在其上增加请求关联、JSON 日志与审计写入框架。没有队列、模型客户端、鉴权或 Canon 读写。
...
| `backend/` | FastAPI `/healthz` 与 `/version`，加上 request_id / JSON 日志 / 审计写入接口 | 无小说业务逻辑；无 Canon / 鉴权 / 队列 / 模型客户端；应用进程不连库 |
| `backend/alembic/` | `audit_events` 的可审阅迁移 | 无 Canon 表、无 Story Project 表；单元测试不跑迁移 |
| `tests/` | `/healthz`、request_id、审计写入的进程内测试；可收集 `contracts/` 测试 | 无小说业务测试；不需要 live Postgres |
...
- 通用 `AuditWriter` + `AuditSink`（测试用内存 sink）。
- Alembic：仅 `audit_events` 一张表的手写迁移。
- 进程内测试：`/healthz`、request_id、审计写入与脱敏（以及已有的 contracts 校验）。不连 Postgres，不调用模型。

## 4. 本节点明确不是

下列项不属于节点 1.3，不得当作本节点已交付：

- Canon 表、Canon 写入 / 读取逻辑、校验引擎、场景生成。
- Story Project / Story Spec CRUD（节点 2.1）。
- 用户鉴权、队列、模型调用。
- 小说写作业务功能。
