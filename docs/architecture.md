...
| `tests/` | `/healthz`、request_id、审计写入、Story Project / Spec、Canon、Snapshot、Scene Card、LLM Gateway、Scene Plan / Scene Draft 作业、Candidate Change 抽取与人类批准 / 提交、Scene / Chapter 摘要、Validation Run、Repair Task、Context Pack 组装、Outline Revision、Style Guide / Style Sample、Style Validation、审校队列、本地作业队列 / Worker、Agent 注册表 / 权限、单场景 DAG 编排、批量调度、叙事一致性评测的进程内测试；可收集 `contracts/` 测试 | 不需要 live Postgres；无真实模型调用；Gateway / 作业 / 评测测试只用 Fake / 夹具 |
...
## 3. 本仓库包含（节点 1.1 + 1.2 + 1.3 + 2.1 + 2.2 + 2.3 + 3.1 + 3.2 + 3.3 + 3.4 + 4.1 + 4.2 + 4.3 + 5.1 + 5.2 + 6.1 + 6.2 + 7.1 + 7.2 + 7.3 + 8.1 + 8.2 + 8.3 + 8.4 + 9.1）
...
- 叙事一致性评测（节点 9.1）：`evals/cases/`、`evals/fixtures/`、`evals/expected/` 与确定性 runner。九类一致性错误各至少一例。复用 5.x 硬规则；仅当类别无法用既有规则表达时才加 `eval.*` 检查。不写 Canon，不批准，不调真实模型。不是 9.2，不是 9.3。无评测 HTTP 路由。
- 进程内测试：`/healthz`、request_id、审计写入与脱敏、Story Project / Spec、Canon、Snapshot、Scene Card、LLM Gateway、Scene Plan / Scene Draft 作业、Candidate Change 抽取与人类批准 / 提交、Scene / Chapter 摘要、Validation Run、Repair Task、Context Pack 组装、Outline Revision、Style Guide / Style Sample、Style Validation、审校队列、本地作业队列 / Worker、Agent 注册表 / 权限、单场景 DAG、批量调度、叙事一致性评测（以及已有的 contracts 校验）。不连 Postgres，不调用真实模型。

## 4. 本节点明确不是

下列项不属于节点 9.1，不得当作本节点已交付：

- 节点 9.2 Prompt / 模型 / 检索回归实验，或节点 9.3 发布门 / 全书导出。
