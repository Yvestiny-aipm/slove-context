...
节点 P.1 只增加本地书快照（默认 `data/book.json` / `SLOVE_BOOK_PATH`），使 Demo / 服务进程重启仍是上一本已保存的书。不写 Canon，不批准。
...
| `data/` | 本地数据目录占位；节点 P.1 默认书快照 `book.json`（gitignore） | 不存放密钥；正文不进 Git |
...
## 3. 本仓库包含（节点 1.1 + 1.2 + 1.3 + 2.1 + 2.2 + 2.3 + 3.1 + 3.2 + 3.3 + 3.4 + 4.1 + 4.2 + 4.3 + 5.1 + 5.2 + 6.1 + 6.2 + 7.1 + 7.2 + 7.3 + 8.1 + 8.2 + 8.3 + 8.4 + 9.1 + 9.2 + 9.3 + UI.1 Demo + UI.2 穿梭 + UI.3 摘要穿梭 + UI.4 DeepSeek 写稿 + P.1 本地书快照）
...
- 节点 P.1 本地书快照：未注入仓库时，`create_app` 从 `data/book.json`（或 `SLOVE_BOOK_PATH`）加载上一本已保存的书，并在项目 / 场次 / 草稿 / Canon 事实 / 候选写入时 flush。空目录可空书。Demo 不得用夹具覆盖已保存的进口书。快照不含 API Key。不写 Canon，不批准。
...
- 把节点 P.1 书快照写成自动批准 / 写 Canon、第二本书切换器、向量库、或生产 seed-status HTTP。
