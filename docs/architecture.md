...
| `tests/` | `/healthz`、request_id、审计写入、Story Project / Spec、Canon、Snapshot、Scene Card、LLM Gateway、Scene Plan / Scene Draft 作业、Candidate Change 抽取与人类批准 / 提交、Scene / Chapter 摘要、Validation Run、Repair Task、Context Pack 组装、Outline Revision、Style Guide / Style Sample、Style Validation、审校队列、本地作业队列 / Worker、Agent 注册表 / 权限、单场景 DAG 编排的进程内测试；可收集 `contracts/` 测试 | 不需要 live Postgres；无真实模型调用；Gateway / 作业测试只用 Fake / 夹具 |
...
## 3. 本仓库包含（节点 1.1 + 1.2 + 1.3 + 2.1 + 2.2 + 2.3 + 3.1 + 3.2 + 3.3 + 3.4 + 4.1 + 4.2 + 4.3 + 5.1 + 5.2 + 6.1 + 6.2 + 7.1 + 7.2 + 7.3 + 8.1 + 8.2 + 8.3）
...
- 单场景 DAG 编排：固定节点 context_pack / scene_plan / plan_validation / scene_draft / candidate_extraction / draft_validation / human_review / canon_commit / summary / downstream_unblock。编排器管状态、输入、权限与批准；节点经 8.1 Worker / 8.2 权限无状态执行。并行节点不得写同一资源。blocker 停止后续自动 submit。按节点 rerun 默认复用冻结上游；重建 Context Pack 必须显式。可视化 JSON 含节点 status 与 duration_ms。canon_commit 仅在人类批准后调用既有 4.2 submit。无 8.4 批量。无生产 seed-status。
- 进程内测试：`/healthz`、request_id、审计写入与脱敏、Story Project / Spec、Canon、Snapshot、Scene Card、LLM Gateway、Scene Plan / Scene Draft 作业、Candidate Change 抽取与人类批准 / 提交、Scene / Chapter 摘要、Validation Run、Repair Task、Context Pack 组装、Outline Revision、Style Guide / Style Sample、Style Validation、审校队列、本地作业队列 / Worker、Agent 注册表 / 权限、单场景 DAG（以及已有的 contracts 校验）。不连 Postgres，不调用真实模型。

## 4. 本节点明确不是

下列项不属于节点 8.3，不得当作本节点已交付：
