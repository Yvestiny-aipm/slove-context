...
| `backend/` | FastAPI `/healthz`、`/version`、request_id / JSON 日志 / 审计写入，Story Project / Spec API，最小 Canon API，Canon Snapshot 冻结 / 回放，Scene Card / 顺序 / 依赖，LLM Gateway（Fake Provider），Scene Plan / Scene Draft 生成作业，Candidate Change 抽取作业，以及候选变更的人类批准 / 拒绝 / 提交 | 无鉴权 / 队列 / 真实模型客户端；无 Context Pack 组装器 / Validate；单元测试用内存仓库，不连库 |
| `backend/alembic/` | `audit_events`、Story Project / Spec 与 Canon 表的可审阅迁移；2.3 增量加 snapshot 列；3.1 建 `arcs` / `chapters` / `scenes` / `scene_dependencies`；3.3 建 `scene_plan_jobs` / `scene_plans`；3.4 建 `scene_draft_jobs` / `scene_drafts`；4.1 建 `extract_jobs` / `candidate_changes`；4.2 在 `candidate_changes` 上增量加批准 / 提交列 | 单元测试不跑迁移；不重建 Canon / 项目 / Scene Card / Scene Plan / Scene Draft / extract 表；不建 Validation Run 表 |
| `prompts/` | 带版本号的 Scene Plan / Scene Draft / Candidate Extract Prompt 模板 | 不是 Context Pack 组装 Prompt，也不是 Validate Prompt |
| `tests/` | `/healthz`、request_id、审计写入、Story Project / Spec、Canon、Snapshot、Scene Card、LLM Gateway、Scene Plan / Scene Draft 作业、Candidate Change 抽取与人类批准 / 提交的进程内测试；可收集 `contracts/` 测试 | 不需要 live Postgres；无真实模型调用；Gateway / 作业测试只用 Fake / 夹具 |
...
## 3. 本仓库包含（节点 1.1 + 1.2 + 1.3 + 2.1 + 2.2 + 2.3 + 3.1 + 3.2 + 3.3 + 3.4 + 4.1 + 4.2）
...
- 进程内测试：`/healthz`、request_id、审计写入与脱敏、Story Project / Spec、Canon、Snapshot、Scene Card、LLM Gateway、Scene Plan / Scene Draft 作业、Candidate Change 抽取与人类批准 / 提交（以及已有的 contracts 校验）。不连 Postgres，不调用真实模型。

## 4. 本节点明确不是

下列项不属于节点 4.2，不得当作本节点已交付：

- Validate / Validation Run（5.x）、节点 4.3、Scene / Chapter 摘要、Context Pack 组装器。
