# 架构（节点 1.1 骨架 + 节点 1.2 本地环境 + 节点 1.3 审计日志 + 节点 2.1 Story Project / Spec）

本文冻结本地单体仓库的目录边界。节点 1.2 补可运行的 Postgres 与 FastAPI 健康检查。节点 1.3 只加请求 `request_id`、JSON 结构化日志、以及 `audit_events` 迁移与通用写入接口。节点 2.1 增加 Story Project / Story Spec / Revision 持久化与 API。  
...
节点 1.1 提供目录与约定。节点 1.2 增加可启动的 Postgres 与仅含 `/healthz`、`/version` 的 FastAPI。节点 1.3 在其上增加请求关联、JSON 日志与审计写入框架。节点 2.1 增加唯一 Story Project 与 Story Spec 草稿 / 提交 / 人工批准 / 修订版本。没有队列、模型客户端、鉴权或 Canon 读写。
...
| `docs/audit.md` | 审计表、JSON 日志与脱敏策略 | 不表示已有 Canon 或业务审计写入路径 |
| `contracts/` | 已批准 JSON Schema（节点 0.4） | 不表示已有读写这些对象的服务 |
| `backend/` | FastAPI `/healthz`、`/version`、request_id / JSON 日志 / 审计写入，以及 Story Project / Spec API | 无 Canon 读写；无鉴权 / 队列 / 模型客户端；单元测试用内存仓库，不连库 |
| `backend/alembic/` | `audit_events` 与 `story_projects` / `story_specs` / `story_spec_versions` 的可审阅迁移 | 无 Canon 表；单元测试不跑迁移 |
| `tests/` | `/healthz`、request_id、审计写入、Story Project / Spec 的进程内测试；可收集 `contracts/` 测试 | 不需要 live Postgres；无模型调用 |
...
- Alembic：`audit_events`（1.3）以及 `story_projects` / `story_specs` / `story_spec_versions`（2.1）手写迁移。
...
- 进程内测试：`/healthz`、request_id、审计写入与脱敏、Story Project / Spec（以及已有的 contracts 校验）。不连 Postgres，不调用模型。

## 4. 本节点明确不是

下列项不属于节点 2.1，不得当作本节点已交付：
