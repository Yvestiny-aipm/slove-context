...
| `tests/` | `/healthz`、request_id、审计写入、Story Project / Spec、Canon、Snapshot、Scene Card、LLM Gateway、Scene Plan / Scene Draft 作业、Candidate Change 抽取与人类批准 / 提交、Scene / Chapter 摘要、Validation Run、Repair Task、Context Pack 组装、Outline Revision、Style Guide / Style Sample、Style Validation、审校队列、本地作业队列 / Worker、Agent 注册表 / 权限、单场景 DAG 编排、批量调度、叙事一致性评测、实验运行与基线对比、发布门与全书导出、以及 UI.1 Demo 播种 / CORS 的进程内测试；可收集 `contracts/` 测试 | 不需要 live Postgres；无真实模型调用；Gateway / 作业 / 评测 / 实验 / 发布 / Demo 测试只用 Fake / 夹具 |
| `frontend/` | 节点 UI.1 本地工作流 Demo：Vite + React + TypeScript；只读已落地 2.1–9.3 API，审校按钮打现有 7.3 / 4.2 | 不是 10.x；无登录鉴权；无真实模型；无设计系统；批准不自动提交 Canon |
...
## 3. 本仓库包含（节点 1.1 + 1.2 + 1.3 + 2.1 + 2.2 + 2.3 + 3.1 + 3.2 + 3.3 + 3.4 + 4.1 + 4.2 + 4.3 + 5.1 + 5.2 + 6.1 + 6.2 + 7.1 + 7.2 + 7.3 + 8.1 + 8.2 + 8.3 + 8.4 + 9.1 + 9.2 + 9.3 + UI.1 Demo）
