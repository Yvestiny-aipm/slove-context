...
节点 1.1 建立了本地单体仓库骨架。节点 1.2 在此基础上提供可运行的本地 Postgres 与 FastAPI `/healthz` / `/version`。节点 1.3 增加请求 `request_id`、JSON 结构化日志、以及 `audit_events` 迁移与通用审计写入接口。本节点没有 Canon、用户鉴权、队列、模型调用，也没有 Story Project / Story Spec（那是 2.1）。
...
| `backend/` | FastAPI 应用（`/healthz`、`/version`、request_id、JSON 日志、审计写入接口） |
| `backend/alembic/` | 可审阅的 `audit_events` 迁移（单元测试不连库） |
| `tests/` | `/healthz`、request_id、审计写入的进程内测试；`make test` 也会跑 contracts 测试 |
| `scripts/` | 脚本目录占位 |
...
等到 `postgres` 为 healthy。数据落在命名卷 `postgres_data`。本节点不建 Canon 表。`audit_events` 用 Alembic 建（见下方「迁移」）；单元测试不跑迁移、不连库。
...
节点 1.1 的骨架步骤（克隆、复制 `.env.example`、venv、`make test`）仍然适用；1.2 在其上增加了 Postgres 与上述两个路由；1.3 增加 request_id、JSON 日志与审计框架。
...
迁移文件在 `backend/alembic/versions/`。节点 1.3 只有 `001_create_audit_events.py`（列：`id`、`occurred_at`、`actor_type`、`actor_id`、`action`、`resource_type`、`resource_id`、`before_json`、`after_json`、`correlation_id`）。
...
`tests/test_healthz.py` 用 FastAPI `TestClient` 在进程内检查 `/healthz`（以及 `/version`）。`tests/test_request_id.py` 检查 `X-Request-ID` 与 JSON 请求日志字段。`tests/test_audit.py` 检查审计写入与脱敏。不启动 Docker，不调用模型。同一命令会收集 `contracts/` 下已批准 Schema 的校验测试。
